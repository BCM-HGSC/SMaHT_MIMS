...
def get_scores(df, state="state", features=["svtype","szbin"], min_obs=10):
    ...
    if filtered.any():
        logging.warning("%d bins with < %d observations ignored.", filtered.sum(), min_obs)
...
def parse_args(args):
    ...
    parser.add_argument("--preset", type=str, default=None, choices=known_features.keys(), 
                        help="Preset features to score (default:mims)")
    parser.add_argument("--features", type=str, default=None,
                        help="Comma-separated list of DataFrame column names of features to score.")
    ...
    parser.add_argument("-c", "--count-output", type=str, default=None,
                        help="Write full observed bin count tsv file (False)")
    # Maybe group these
    ...
    parser.add_argument("--tail", type=str, default="left", choices=['left', 'right', 'two'],
                        help="Tail to test (%(default)s)")
    parser.add_argument("--alpha", type=truvari.restricted_float, default=0.01, 
...
def set_mims_bins(data):
    """
    Automatic stratification of DataFrame columns based when they contain SMaHT MIMS variants
    """
    def get_vaf_bin(vaf, bins=[0, 0.05, 0.3, 0.7, 1], labels=["SomaticLow(<5%)", "Somatic", "GermlineHet", "GermlineHom"]):
        """
    ...
    data['VAF_bin'] = get_vaf_bin(data['VAF_alt'])
...
if __name__ == '__main__':
    ...
    data_file = os.path.join(args.bench, "data.jl")
    if not os.path.exists(data_file):
        logging.info("Constructing DataFrame in %s", data_file)
        vcf2df_main(f"-i -f -b {args.bench} {data_file}".split(' '))
    ...
    scores.round(4).sort_values(by=['adj_pval'], ascending=True).to_csv(args.output, sep='\t', index=False)
