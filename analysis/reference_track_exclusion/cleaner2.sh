...
cat toobig.bed tooseg.bed gap.slop.bed phab.failed.bed phab.didnot.help.bed \
    | cut -f1-3 | bedtools sort | bedtools merge > to_exclude.bed

#awk '$4 > 3' tmp.mediumchunks.bed| bedtools slop -b 100 -g ~/code/references/grch38/GRCh38_1kg_mainchrs.fa | bedtools merge > mediumchunk.dense3.bed
full_bed=smaht_mims_sv_v2_easy_fullregions.bed
nodense_bed=smaht_mims_sv_v2_easy_nodenseregions.bed
vcf=../benchmark_v2/smaht_mims_sv_v2_easy.vcf.gz
#vcf=../benchmark_v2/smaht_mims_sv_v2_hard.vcf.gz
bedtools subtract -a $reg -b to_exclude.bed | grep -v chrX | grep -v chrY > ${full_bed}

python make_simple.py ${vcf} ${full_bed} > split_var_candidates.bed
...
bedtools subtract -a ${full_bed} -b split_var_candidates.slop.bed > ${nodense_bed}
